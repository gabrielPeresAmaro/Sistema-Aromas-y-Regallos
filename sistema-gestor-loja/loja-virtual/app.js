...
//rotas:
    ...
    app.post('/produtos/cadastrar', function(req, res){
        ...
        const { nome, descricao, quantidade_estoque, preco, categoria, foto } = req.body;

        if (nome == "" || descricao == ""){
            res.send("Erro - não pode haver campo de dado em branco.");
        }
        else {
            var umProdutoRecebido = req.body;
    ...
    app.post('/produtos/salvar-edicao', function(req, res){
        ...
        if (nome == "" || descricao == ""){
            ...
            var umProdutoRecebido = req.body;
            Produto.update( 
                { nome, descricao, quantidade_estoque, preco, categoria, foto },
                { where: {codigo} }
             );
